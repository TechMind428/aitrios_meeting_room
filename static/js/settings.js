...
function getCommandParametersFromForm() {
    try {
        ...
        // デバイスIDを置換
        storageSubDir = storageSubDir.replace('{device_id}', deviceId);
        storageSubDirIR = storageSubDirIR.replace('{device_id}', deviceId);
...
function toggleInference(deviceId, enable) {
    ...
    console.log(`Toggling inference for device ${deviceId} to ${enable ? 'on' : 'off'}`);
    ...
    .then(response => response.json())
    .then(data => {
        console.log('Toggle inference response:', data);
